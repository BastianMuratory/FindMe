...
#include "src/chargement/chargement.h" // chargement des données
...
int main(void) {
	...
	bool admin = false; // mode administrateur ou pas 
	int x; // sert pour le choix de la commande dans le menu

	// valeur du parramètre d'indexation choisit dernièrement
	int vT = -1; 
	int vI = -1;
	int vA = -1;
  ...
  idAudio = 0;
	...
	*/
	...
	do{
		...
		// pour éviter de surcharger le terminal
		system("clear");
		...
		switch (x){
			...
			case 2:
				...
				if(peut_chercher_texte){
					...
					// si le parramètre d'indexation à été changé 
          ...
          rechercher_audio(&pile_descripteur_audio,&Liens_Audio,derniere_ind_audio,nombre_resultats_audio);
			...
			case 6:
				if(admin){
					// changer les parramètres d'indexation de texte
			...
			case 7:
				if(admin){
					// changer les parramètres d'indexation des images
			...
			case 8:
				if(admin){
					// changer les parramètres d'indexation des fichiers audios
	...
	// sauvegarde des derniers parramètres d'indexation selectionnés
	...
	// sauvegarde des parramètres utilisés lors de la dernière sauvegarde
