...
#define CHEMIN "./data/Textes/"  //Répertoire : FindMe
...
/* retourne la liste de termes passées en paramètre en ajoutant le nouveau token */
...
/* Ajoute la cellule à la ième position */
...
/* initialise un descripteur avec le nom du fichier et son identifiant */
...
/* i correspond à l'indice le plus à droite du tableau tab */
...
int nettoyage(FILE* fic_in, FILE* fic_out) {
  //printf("*Nettoyage du fichier en cours... ");
  //fflush(stdout);
...
int filtrage(FILE* fic_in, FILE* fic_out) {
  //printf("*Suppretion des token du fichier en cours... ");
  //fflush(stdout);
  char buffer[TAILLE_MAX+1];
...
DescripteurTexte chargerDescripteur(FILE* fic_in, int nb_termes, char* nomFic, int ID) {
  //printf("*Chargement du descripteur en cours... ");
  //fflush(stdout);
  int i;
...
// Creation du descripteur avec nmbDeMots définit par l'utilisateur
...
/* copie en profondeur de e1 dans e2 */
...
int sauvegarderDescripteurTexte(FILE* fichier,DescripteurTexte d){
	fprintf(fichier, "%s ;%d;%d;%d;", d.nom, d.ID, d.nbTermes, d.nbTokenFic);
  Cellule* celCur = d.termes.tete;
  while(celCur != NULL) {
    fprintf(fichier, "%s %d ", celCur->token, celCur->nbOcc);
    celCur = celCur->suiv;
  }
  fprintf(fichier, "\n");
	return 0;
